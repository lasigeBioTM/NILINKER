...
class Words2embed(tf.keras.layers.Layer):
    ...
    def __init__(self, wc, word_embeds, candidate_embeds, kb_data):
...
class Nilinker(tf.keras.Model):
    ...
    def __init__(self, word_embeds, candidate_embeds, 
        ...
        self.id_to_name = kb_data.id_to_name
        self.top_k= params[2]
    ...
    def train_step(self, data):
        ...
        y_true_bin_tmp = tf.keras.utils.to_categorical(
                            ...
                            dtype='float32')
        y_true_bin = tf.convert_to_tensor(y_true_bin_tmp, dtype=tf.float32)
         
        with tf.GradientTape(persistent=True) as tape:
            y_pred, top_candidates = self(x_batch) 
        ...
        self.compiled_metrics.update_state(y_true_bin, y_pred) 
    ...
    def predict_step(self, input):
        ...
        top_candidates_names = [
            self.id_to_name[kb_id] for kb_id in top_candidates_kb_ids]
